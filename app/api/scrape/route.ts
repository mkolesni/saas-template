...
export async function POST(request: NextRequest) {
  try {
    ...
    const sqft = scraped.data.metadata?.sqft || '2,500 sqft';
    const description = scraped.data.content || scraped.data.description || 'Stunning home with premium features';
    const images = scraped.data.images || ['https://images.unsplash.com/photo-1600585154340-be6161a56a0c']; // Real listing photos
    ...
    // 3. Runway — FORCED TO USE LISTING PHOTOS + DATA
    const runwayRes = await fetch('https://api.dev.runwayml.com/v1/text_to_video', {
      ...
      body: JSON.stringify({
        model: 'veo3.1',
        promptText: `Award-winning luxury real estate tour for ${title}. Use ONLY these listing photos: ${images.slice(0, 6).join(', ')}. Flash text overlays: "${price} | ${bedsBaths} | ${sqft}". Smooth cinematic drone pans, golden hour lighting, marble interiors sparkling, ocean views, high-end furniture, elegant transitions, professional film look, subtle music, voiceover sync. Make it look like a $5,000 listing video — nothing else.`,
        ratio: '1080:1920',
        duration: 8,
        audio: true,
      }),
