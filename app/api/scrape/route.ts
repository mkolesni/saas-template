...
export async function POST(request: NextRequest) {
  try {
    ...
    const voiceRes = await fetch('https://api.elevenlabs.io/v1/text-to-speech/21m00Tcm4TlvDq8ikWAM', {
      ...
      body: JSON.stringify({
        text: `Welcome to ${title}. ${description.substring(0, 800)}. Call today!`,
      }),
    });

    const audioBlob = await voiceRes.blob();
    const audioBase64 = Buffer.from(await (await audioBlob).arrayBuffer()).toString('base64');
    ...
    const runwayRes = await fetch('https://api.runwayml.com/v1/tasks', {
      ...
      body: JSON.stringify({
        model: 'gen-4-turbo',
        input: {
          promptText: `Luxury real estate tour for ${title}. Smooth pans, golden hour, elegant overlays, professional voiceover.`,
          ratio: '9:16',
          duration: 60,
          image_url: image,
          audio_url: audioUrl,
    ...
    if (task.id) {
      ...
      for (let i = 0; i < 20; i++) {
        ...
        if (result.status === 'succeeded' && result.output?.[0]) && (videoUrl = result.output[0]) && break;
      }
    }
